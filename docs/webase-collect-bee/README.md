...
数据导出组件WEBASE-BEE的目的在于降低获取区块链数据的开发门槛，提升研发效率。研发人员几乎不�?要编写任何代码，只需要进行简单配置，就可以把数据导出到Mysql数据库�??

WEBASE-BEE可以导出区块链上的基�?数据，如当前块高、交易�?�量等�?�如果正确配置了FISCO-BCOS上运行的�?有合约，WEBASE-BEE可以导出区块链上这些合约的业务数据，包括event、构造函数�?�合约地�?、执行函数的信息等�??

数据导出组件支持多数据源、分库分表�?�读写分离�?�分布式部署�?

WEBASE-BEE提供了基于Restful的API，支持�?�过http的方式调用这些接口�??

WEBASE-BEE还集成了Swagger组件，提供了可视化的文档和测试控制台�?

你可以�?�过[WEBASE-MONKEY](https://github.com/WeBankFinTech/webase-monkey/tree/feature_datacheck.2019.03)来自动生成本工程，只�?要在�?个配置文件中进行少量�?单的配置，同时按照要求提供相关的智能合约信息；我们推荐这种方式�??
...
该公司使用[WEBASE-MONKEY](https://github.com/WeBankFinTech/webase-monkey/tree/feature_datacheck.2019.03)迅�?�生成了webase-bee的代码，并根据实际需求进行了定制化开发，在一天之内投入到线上使用�?
...
- 解决方案
该公司使用[WEBASE-MONKEY](https://github.com/WeBankFinTech/webase-monkey/tree/feature_datacheck.2019.03)迅�?�生成了webase-bee的代码，并根据实际需求进行了定制化开发�?��?�过在智能合约中设计的各类event，相关的业务数据都被导出到数据库中；从�?�实现轻松对账的�?求�??
...
该公司使用[WEBASE-MONKEY](https://github.com/WeBankFinTech/webase-monkey/tree/feature_datacheck.2019.03)迅�?�生成了webase-bee的代码，并根据实际需求进行了定制化开发，区块链上的数据可以实时导出到数据库中。利用webase-bee自带的Restful API，该公司的报表系统实现了和区块链数据的对接，可以获得准实时的各类业务报表�?
...
webase-bee的工程使用gradle进行构建，是�?个SpringBoot工程�?
...
当完整地按照[WEBASE-MONKEY](https://github.com/WeBankFinTech/webase-monkey/tree/feature_datacheck.2019.03)的操作手册进行操作获得webase-bee工程后，会得到webase-bee工程，主要的基础配置都将会在配置中自动生成，无需额外配置。但是，基于已生成的配置文件，你可以继续按照�?求进行深入的个�?�化高级配置，例如配置集群部署�?�分库分表�?�读写分离等等�??
...
sharding.jdbc.datasource.master.jdbc-url=jdbc:mysql://127.0.0.1:3306/test0?useSSL=false&serverTimezone=GMT%2b8
sharding.jdbc.datasource.master.username=
sharding.jdbc.datasource.master.password=
...
sharding.jdbc.datasource.slave0.jdbc-url=jdbc:mysql://127.0.0.1:3306/test1?useSSL=false&serverTimezone=GMT%2b8
sharding.jdbc.datasource.slave0.username=
sharding.jdbc.datasource.slave0.password=
...
supervisor的安装与部署可以参�?? [WEBASE-MONKEY](https://github.com/WeBankFinTech/webase-monkey/tree/feature_datacheck.2019.03) 附录6的说明文档�??
...
#### 2.2.5 监控数据导出
在webase-bee工程�?终编译后，生成的dist目录会有个monitor.sh脚本.  执行该脚本可以用来监控数据导出服务是否正常启动或者数据导出是否正常工作�??
...
### 2.4 �?启可视化的功能�?�测�?
WEBASE-BEE默认集成了swagger的插件，支持通过可视化的控制台来发�?�交易�?�生成报文�?�查看结果�?�调试交易等�?
...
例如，当你在本机运行了webase-bee，且未修改默认的8080端口，则可以访问此地�?�?
