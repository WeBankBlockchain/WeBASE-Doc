...
数据导出组件WeBASE-Collect-Bee的目的在于降低获取区块链数据的开发门槛，提升研发效率。研发人员几乎不�?要编写任何代码，只需要进行简单配置，就可以把数据导出到Mysql数据库�??

WeBASE-Collect-Bee可以导出区块链上的基�?数据，如当前块高、交易�?�量等�?�如果正确配置了FISCO-BCOS上运行的�?有合约，WeBASE-Collect-Bee可以导出区块链上这些合约的业务数据，包括event、构造函数�?�合约地�?、执行函数的信息等�??

数据导出组件支持多数据源、分库分表�?�读写分离�?�分布式部署�?

WeBASE-Collect-Bee提供了基于Restful的API，支持�?�过http的方式调用这些接口�??

WeBASE-Collect-Bee还集成了Swagger组件，提供了可视化的文档和测试控制台�?

你可以�?�过[WeBASE-Codegen-Monkey](https://github.com/WeBankFinTech/WeBASE-Codegen-Monkey/tree/dev_v0.7.0.2019.06)来自动生成本工程，只�?要在�?个配置文件中进行少量�?单的配置，同时按照要求提供相关的智能合约信息；我们推荐这种方式�??

### 1.2 使用场景和解决方�?

区块链的数据存储在区块链上，�?要使用智能合约暴露的接口来进行调用�?�由于智能合约暴露的接口的限制，区块链上不�?�合进行复杂的数据查询�?�大数据分析和数据可视化等工作�?�因此，我们致力于提供一种智能化、自动化的数据导出和备份的解决方案�??

#### 案例 数据可视化后台系�?

- 背景

某互联网小贷公司基于FISCO-BCOS�?发了区块链�?�条业务系统，客户之间的借贷合同信息和证明材料都会在脱敏后保存到区块链上。该公司的运营人员需要获得当前业务进展的实时信息和摘要信息�??

- 解决方案

该公司使用[WeBASE-Codegen-Monkey](https://github.com/WeBankFinTech/WeBASE-Codegen-Monkey/tree/dev_v0.7.0.2019.06)迅�?�生成了WeBASE-Collect-Bee的代码，并根据实际需求进行了定制化开发，在一天之内投入到线上使用�?

导出到db的数据接入到了该公司的统�?监控平台，该公司PM可以在业务后台系统上获得该业务的实时进展，该公司运维人员可以在公司运维监控室的大屏幕实时监控业务系统的状态�??

#### 案例 区块链业务数据对账系�?

- 背景

某公司基于FISCO-BCOS�?发了区块链的业务系统，需要将本地数据与链上的数据进行对账�?

- 解决方案

该公司使用[WeBASE-Codegen-Monkey](https://github.com/WeBankFinTech/WeBASE-Codegen-Monkey/tree/dev_v0.7.0.2019.06)迅�?�生成了WeBASE-Collect-Bee的代码，并根据实际需求进行了定制化开发�?��?�过在智能合约中设计的各类event，相关的业务数据都被导出到数据库中；从�?�实现轻松对账的�?求�??

#### 案例 区块链业务数据查询系�?

- 背景

某互联网公司基于FISCO-BCOS�?发了区块链的业务系统，但是发现智能合约对业务报表的支持不佳�?�但是，公司的一线业务部门要求实时查看各类复杂的业务报表�?

- 解决方案

该公司使用[WeBASE-Codegen-Monkey](https://github.com/WeBankFinTech/WeBASE-Codegen-Monkey/tree/dev_v0.7.0.2019.06)迅�?�生成了WeBASE-Collect-Bee的代码，并根据实际需求进行了定制化开发，区块链上的数据可以实时导出到数据库中。利用WeBASE-Collect-Bee自带的Restful API，该公司的报表系统实现了和区块链数据的对接，可以获得准实时的各类业务报表�?
...
集成sharding-jdbc组件，支持多数据源�?�分库分表�?�读写分�?

#### 支持集群部署和分布式任务调度

集成elstic-job�?源组件，支持灵活的分布式部署和任务调�?

#### 可定制化的数据导出策�?

提供灵活的可配置的区块�?�交易�?�事件�?�账户等数据导出功能，过滤不�?要的数据
...
| Bash | �?支持Bash（理论上来说支持�?有ksh、zsh等其他unix shell，但未测试）|
| Java | >= JDK[1.8] | |
| Git | 下载的安装包使用Git | |
| MySQL | >= mysql-community-server[5.7] | |
| zookeeper | >= zookeeper[3.4] | 只有在进行集群部署的时�?�需要安装|
...
WeBASE-Collect-Bee的工程使用gradle进行构建，是�?个SpringBoot工程�?
...
自动生成的Java代码�?般位于src/main/java/com/webank/webasebee/generated；�?�合约编译后的文件除了会被存放到config/contract文件夹下以外，还会按照原有的package名称放入到src/main/java的路径下�?

#### 2.2.2 配置工程(更多高级配置)

当完整地按照[WeBASE-Codegen-Monkey](https://github.com/WeBankFinTech/WeBASE-Codegen-Monkey/tree/dev_v0.7.0.2019.06)的操作手册进行操作获得WeBASE-Collect-Bee工程后，会得到WeBASE-Collect-Bee工程，主要的基础配置都将会在配置中自动生成，无需额外配置。但是，基于已生成的配置文件，你可以继续按照�?求进行深入的个�?�化高级配置，例如配置集群部署�?�分库分表�?�读写分离等等�??
...
注意�? 当修改完配置文件后，�?要重新编译代码，然后再执行，编译的命令如下：
...
| 配置�? | 是否必输 | 说明 | 举例 | 默认�? |
| --- | --- | --- | --- | --- |
| system.startBlockHeight | N | 设置导出数据的起始区块号，优先以此配置为�? | 1000 | 0 |
| system.startDate | N | 设置导出数据的起始时间，例如设置导出2019年元旦开始上链的数据；如已配置startBlockHeight，以导出数据起始区块号为准�?�支持的数据格式包括：yyyy-MM-dd HH:mm:ss �? yyyy-MM-dd �? HH:mm:ss �? yyyy-MM-dd HH:mm  �? yyyy-MM-dd  HH:mm:ss.SSS | 2019-01-01 | - |

##### 单节点部署的配置

在�?�择单节点配置后，以下配置会自动生成�?
单节点任务调度的配置，分布式任务调度的配置默认位�? src/main/resources/application.properties

```
#### 当此参数为false时，进入单节点任务模�?
system.multiLiving=false

#### �?启多线程下载的区块阈值，如果当前已完成导出的区块高度小于当前区块总高度减去该阈�?�，则启动多线程下载
...
多节点任务调度的配置，分布式任务调度的配置默认位�? src/main/resources/application.properties
...
#### 分片序列号和参数用等号分隔，多个键�?�对用�?�号分隔,分片序列号从0�?始，不可大于或等于作业分片�?�数
...
实践表明，当区块链上存在海量的数据时，导出到单个数据库或单个业务表会对运维�?�成巨大的压力，造成数据库�?�能的衰减�??
�?般来讲，单一数据库实例的数据的阈值在1TB之内，单�?数据库表的数据的阈�?�在10G以内，是比较合理的范围�??

如果数据量超过此阈�?�，建议对数据进行分片�?�将同一张表内的数据拆分到多个或同个数据库的多张表�??
...
#### 数据库表block_tx_detail_info的配置，以下配置即为数据自动分为5个分片，以block_height%5来进行路由，pk-id为自增�??
...
#### 如果�?要对更多的数据库表进行分片，请按上面的例子进行修改�?�配�?

```

##### 数据库读写分离的配置�?

数据库读写分离的主要设计目标是让用户无痛地使用主从数据库集群，就好像使用�?个数据库�?样�?�读写分离的特�?�支持往主库写入数据，往从库查询数据，从而减轻数据库的压力，提升服务的�?�能�?
...
sharding.jdbc.datasource.master.jdbc-url=jdbc:mysql://127.0.0.1:3306/test0?useSSL=false&serverTimezone=GMT%2b8&useUnicode=true&characterEncoding=UTF-8
sharding.jdbc.datasource.master.username=
sharding.jdbc.datasource.master.password=
...
sharding.jdbc.datasource.slave0.jdbc-url=jdbc:mysql://127.0.0.1:3306/test1?useSSL=false&serverTimezone=GMT%2b8&useUnicode=true&characterEncoding=UTF-8
sharding.jdbc.datasource.slave0.username=
sharding.jdbc.datasource.slave0.password=
...
当然，你也可以使用supervisor来守护和管理进程，supervisor能将�?个普通的命令行进程变为后台daemon，并监控进程状�?�，异常�?出时能自动重启�??

它是通过fork/exec的方式把这些被管理的进程当作supervisor的子进程来启动，这样只要在supervisor的配置文件中，把要管理的进程的可执行文件的路径写进去即可�?

也实现当子进程挂掉的时�?�，父进程可以准确获取子进程挂掉的信息的，可以�?�择是否自己启动和报警�??

supervisor还提供了�?个功能，可以为supervisord或�?�每个子进程，设置一个非root的user，这个user就可以管理它对应的进程�??

supervisor的安装与部署可以参�?? [WeBASE-Codegen-Monkey](https://github.com/WeBankFinTech/WeBASE-Codegen-Monkey/tree/dev_v0.7.0.2019.06) 附录6的说明文档�??
...
你也可以通过DB来检查，登录你之前配置的数据库，看到自动创建完表的信息，以及表内�?始出现数据内容，则代表一切进展顺利�?�如你可以执行以下命令：
```
# 请用你的配置信息替换掉[]里的配置，并记得删除[]
mysql -u[用户名] -p[密码] -e "use [数据库名]; select count(*) from block_detail_info"
```
如果查询结果非空，出现类似的如下记录，则代表导出数据已经�?始运行：
```
+----------+
| count(*) |
+----------+
...
在WeBASE-Collect-Bee工程�?终编译后，生成的dist目录会有个monitor.sh脚本.  执行该脚本可以用来监控数据导出服务是否正常启动或者数据导出是否正常工作�??

##### 2.2.5.0 个�?�化配置

打开monitor.sh，可以修改相关的个�?�化配置�?

```
# WeBASE-Collect-Bee服务启动的IP地址
ip=127.0.0.1
# WeBASE-Collect-Bee服务启动的端�?
port=8082
# 数据导出的进度落后于链高度的报警阈�?�；如当前进度落后当前链高度达到20个以上，输出报警日志�?
threshold=20
# 当链高度增长，数据导出完成块高的报警阈�?�；如当前块高增长，但完成导出的区块数量增长小于等于1.
warn_number=1
```
...
##### 2.2.5.3  配置crontab
 ...
 建议将monitor.sh添加到crontab中，设置为每分钟执行�?次，并将输出重定向到日志文件。可以日常扫描日志中的```ERROR!```字段就能找出节点服务异常的时�?, 也可以在节点挂掉情况下及时将节点重启�?  
 在crontab的配置可以参考如下：
...
首先，请安装docker，docker的安装可参�?�[docker安装手册](https://docker_practice.gitee.io/install/centos.html)
...
如果你是使用sudo用户安装了docker，可能会提示『permission denied』的错误，建议执�?:
```
...
```
grafana将自动绑�?3000端口并自动安装时钟和Json的插件�??
...
您可以�?�择右上方的时间按钮来�?�择和设置时间范围及刷新时间等�??

您也可以选中具体的页面组件进行编辑，自由地移除或挪动组件的位置，达到更好的使用体验�??

更多关于Grafana的自定义配置和开发文档，可参�? [Grafana官方文档](http://docs.grafana.org/guides/getting_started/)
...
例如，当你在本机运行了WeBASE-Collect-Bee，且未修改默认的8080端口，则可以访问此地�?�?

> http://localhost:8080/swagger-ui.html

此时，你可以看到上述页面，可以看到页面主要包括了http请求页面和数据模型两部分�?

#### 2.4.2 使用swagger发�?�具体的交易�?

选择点击对应的http请求集，可以点开相关的http请求。此时，你可以�?�择点击“try it out”，手动修改发�?�的Json报文，点击�?�Excute”按钮，即可发�?�并查收结果�?
...
存储了所有区块的状�?�信息和下载情况，对应数据库表名称为**block_task_pool**,如下�?�?:

| 字段 | 类型 | 字段设置 | 默认�? | 说明 |
| --- | --- | --- | --- | --- |
| pk_id | bigint(20) | Primary key & NOT NULL | 自增 | 主键Id |
| block_height | bigint(20) |  |  | 块高 |
| certainty   |  int(11)		| 是否可能分叉 | 0- 是； 1-�? |
...
区块详细数据存储模型用于存储每个区块的详细数据，包括区块哈希、块高�?�出块时间�?�块上交易量，对应的数据库表名为**block_detail_info**。如下表�?示�??
...
| tx_count | int(11) |  |  | 当前区块交易�? |
| depot_updatetime | datetime |  | 系统时间 | 记录插入/更新时间 |

#### 3.1.3区块交易数据存储模型

区块交易数据存储模型用于存储每个区块中每个交易的基本信息，包括区块哈希�?�块高�?�出块时间�?�合约名称�?�方法名称�?�交易哈希�?�交易发起方地址、交易接收方地址，对应的数据库表名为**block_tx_detail_info**。如下表�?示�??
...
| block_height | bigint(20) |  |  | 区块高度 |
| block_tiemstamp | datetime | index |  | 出块时间 |
| contract_name | varchar(225) |  |  | 该笔交易的合约名�? |
| method_name | varchar(225) |  |  | 该笔交易调用的function名称 |
| tx_hash | varchar(225) |  |  | 交易哈希 |
| tx_from | varchar(225) |  |  | 交易发起方地�? |
| tx_to | varchar(225) |  |  | 交易接收方地�? |
| depot_updatetime | datetime |  | 系统时间 | 记录插入/更新时间 |

### 3.2 账户数据存储模型

账户数据存储模型用于存储区块链网络中�?有账户信息，包括账户创建时所在块高�?�账户所在块的出块时间�?�账户地�?（合约地�?）�?�合约名称�?�对应的数据库表名为**account_info**。如下表�?示�?�需要注意的是，如果通过嵌套合约隐式调用构�?�方法，则不会导出�?�比如A合约中�?�过关键字new�?个B合约，则B合约不会导出�?
...
以上述智能合约为例，对应�? **<event-paralist>** 如下�?

| 字段 | 类型 | 字段设置 | 默认�? | 说明 |
| --- | --- | --- | --- | --- |
| user_name | varchar(255) |  |  | 用户�? |
| sex | int |  |  | 性别 |

### 3.4	交易数据存储模型

交易数据存储模型同事件数据存储模型类似，是根据合约中的方法（Function）自动生成的。一个合约中有多少个方法就会生成多少个对应的方法数据存储表�?�该方法指的是实际产生交易的方法（含构�?�方法），不包含事件（Event）方法和查询方法（constant关键字标注）�?
...
交易字段命名规则也是将交易参数字段驼峰命名转化为小写加下划线，不再赘述�?�需要指出的是，对于�?些没有参数的方法，交易数据存储模型没有办法存储，即�?�过无参方法产生的交易明细将无法通过数据导出工具获取到�??
...
�?**3.3.1**中的合约为例，对应的 **<function-paralist>** 如下�?
