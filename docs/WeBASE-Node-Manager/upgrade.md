...
##### 数据表更新
执行webase-node-mgr中`script/upgrade`目录中的`v152_v153.sql`即可更新数据表，可通过`mysql -e source v152_v153.sql`命令执行
- 注：更新前提前备份mysql数据
...
**数据表字段修改**
- `tb_config`配置信息表增加可视化部署的v2.8.0的配置信息
- `tb_node`节点信息表增加了`city`字段，类型为`varchar(64)`，默认为空
- `tb_node`节点信息表增加了`agency`字段，类型为`varchar(64)`，默认为空
- `tb_node`节点信息表`node_name`字段长度更新为`varchar(255)`
