...
WeBASE控制台是由三个WeBASE子系统组成的一套管理FISCO-BCOS联盟链的工具集。
...
WeBASE控制台分为三个部分：节点前置，节点管理，WEB管理台。
...
节点前置使用Spring Boot的JAVA后台服务，具体搭建流程参见[《节点前置安装说明》](https://github.com/WeBankFinTech/webase-node-mgr/blob/dev-0.5/README.md#chapter-3)。

##### 2.2.2.2、节点管理搭建

节点管理使用Spring Boot的JAVA后台服务，具体搭建流程参见[《节点管理安装说明》](https://github.com/WeBankFinTech/webase-node-mgr/blob/dev-0.5/README.md#chapter-3)。

##### 2.2.2.3、WEB管理台搭建

WEB管理台使用框架`vue-cli`，具体搭建流程参见[《web管理台安装说明》](https://github.com/WeBankFinTech/webase-web/blob/dev-0.5/README.md)。

## 3、系统初始化配置

服务搭建成功后，可使用网页浏览器访问nginx配置的WEB管理台IP和端口(例如XXX.XXX.XXX.XXX:XXXX)，进入到控制台页面。控制台默认用户为admin，默认密码为Abcd1234（第一次登陆成功后会要求重置密码，请按照密码标准设置一个更加安全的密码）。

### 3.1、添加节点前置

 未初始化节点前置的控制台，会引导去节点管理页面添加节点前置。

![](../../images/webase-console-suit/add_node_front.png)

前置添加完成后，控制台就会开始拉取群组信息和群组的区块信息。此时数据概览页面应该就有数据了。为了解析和审计区块数据，需要把相关的合约和用户导入到控制台。具体看下面两个小节。

### 3.2、合约管理

#### 3.2.1添加合约

控制台提供两种添加合约的方式，一个是新建一个合约，一个是导入已有合约。同时合约编辑器还提供新建目录。用目录的形式管理合约，主要是为了解决同名合约引用的问题。合约添加完成后，需要编译保存。
...
私钥管理提供了新建私钥用户和导入公钥用户两种用户导入方式。第一种方式主要用于新建用户在控制台的合约管理中部署和调用合约。第二种方式主要用于把交易和用户关联起来。

![](../../images/webase-console-suit/user_transaction.png)

## 4、各模块的详细介绍

本小节概要介绍控制台的各个模块，方便大家对WeBASE控制台套件有一个整体的认识。这套工具集主要提供的管理功能有：
...
提供管理台登陆账号的管理功能。管理台用户分为两种角色：普通用户和管理员用户。这两者的区别是，管理员拥有控制台的读写权限，而普通用户只有查看权限。
